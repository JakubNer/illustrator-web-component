(ns kundel.component
  (:require
    [reagent.core :as r]
    [dommy.core :as dom]
    [kundel.css :as css]))

(def attrs (atom {}))

(defn ctor-attrs [this]
  (reset! attrs (assoc @attrs this {:illustration (r/atom nil)
                                    :keyframe (r/atom nil)
                                    :next-hint (r/atom nil)
                                    :current (r/atom nil)
                                    :instance-id (gensym "illustration-instance-")
                                    :id (r/atom (gensym ""))})))

(defn has-attrs [this]
  (contains? @attrs this))

(defn illustration [this] 
  (:illustration (get @attrs this)))

(defn keyframe [this]
  (:keyframe (get @attrs this)))

(defn next-hint [this]
  (:next-hint (get @attrs this)))

(defn current [this]
  (:current (get @attrs this)))

(defn id [this]
  (:id (get @attrs this)))

(defn instance-id [this]
  (:instance-id (get @attrs this)))

;;
;; illustration / timeline variables and utilities
;;

(defn get-illustration
  "Give sections as per component contract returns linked structure {:section ?? :subsection ?? :flow ?? :subsnext ?? :subsprev ??} for each flow from each section and each subsection; in order."
  ([sections]
   (get-illustration
     sections
     (:flows (first sections))
     (:subsections (first sections))
     (:flows (first (:subsections (first sections))))
     nil))
  ([sects flows subs subflows subsprev]
   (if (seq flows)
     (cons {:section (first sects) :subsection nil :flow (first flows) :subsnext nil :subsprev nil :numsubs (count subs)}
           (get-illustration sects (rest flows) subs subflows subsprev))
     (if (seq subflows)
       (cons {:section (first sects) :subsection (first subs) :flow (first subflows) :subsnext (second subs) :subsprev subsprev :numsubs (count subs)}
             (get-illustration sects nil subs (rest subflows) subsprev))
       (if (seq subs)
         (get-illustration sects nil (rest subs) (:flows (second subs)) (first subs))
         (when (seq sects)
           (get-illustration (rest sects))))))))

(defn get-illustration-keyframe [illustration flow]
  "retrieves illustration keyframe for flow."
  (first (keep-indexed #(when (= flow (:flow %2)) %1) illustration)))

(defn get-illustration-keyframe-by-id [illustration id]
  "retrieves illustration keyframe for flow."
  (first (keep-indexed #(when (= id (:id (:flow %2))) %1) illustration)))

(defn get-subsection-index-percentage-tuple [illustration flow]
  "Retrieve 0-based index of flow in subsection, if any, or nil.  Retrieve percentage of all subsections where subsection occurs, so 3rd subsection of 4 subsections is at 75%.  These two values are in a tuple (index, percentage)"
  (let [_keyframe (get-illustration-keyframe illustration flow)
        _current (nth illustration _keyframe)]
    (when-let [subsection (:subsection _current)]
      (let [subscoll (:subsections (:section _current))
            index (first (keep-indexed #(when (= subsection %2) %1) subscoll))]
        (list index (* 100 (/ index (count subscoll))))))))

(defn get-element-id [flow]
  "Retrieve HTML ID for 'flow' element"
  (str "illustrator-flow-" (:id flow)))

(defn find-parent-with-class [js-element with-class]
  "Find a JS element that's a parent of passed in 'js-element' with class 'with-class'."
  (when js-element
    (when-let [parent (dom/parent js-element)]
      (if (dom/has-class? parent with-class)
        parent
        (find-parent-with-class parent with-class)))))

(defn html-collection-to-vec
  "Converts nodelist to (not lazy) seq."
  [hc]
  (into [] (for [k (range (aget hc "length"))]
             (aget hc k))))

(defn find-child-with-class [js-element with-class]
  "Find a JS element that's an immediate child of passed in 'js-element' with class 'with-class'."
  (when js-element
    (when-let [children (html-collection-to-vec (dom/children js-element))]
      (first
        (filter #(if (dom/has-class? % with-class)
                     %
                     nil)
                children)))))

(defn add-remove-classes-and-properties-for-animation [this]
  "go through all IDs and animate as required"
  (let [current-flow (:flow @(current this))
        current-section (:section @(current this))
        current-subsection (:subsection @(current this))
        current-flow-js-element (dom/sel1 (str "#" (get-element-id current-flow)))
        section-js-element (find-parent-with-class current-flow-js-element "illustrator-section")
        sections-js-element (find-parent-with-class current-flow-js-element "illustrator-sections")]
    (doseq [rec @(illustration this)
            :let [flow (:flow rec)
                  flow-js-element (dom/sel1 (str "#"(get-element-id flow)))
                  subsection (:subsection rec)
                  section (:section rec)]
            :when flow-js-element
            :when (not= rec @(current this))]
      (dom/remove-class! flow-js-element "illustrator-current")
      (when (and subsection (not (= subsection current-subsection)))
        (when-let [subsection-js-element (find-parent-with-class flow-js-element "illustrator-subsection")]
          (dom/remove-class! subsection-js-element "illustrator-current"))
        (when-let [subsection-frame-js-element (find-parent-with-class flow-js-element "illustrator-subsection-frame")]
          (dom/remove-class! subsection-frame-js-element "illustrator-current")
          (dom/remove-class! subsection-frame-js-element "has-previous-subsection")
          (dom/remove-class! subsection-frame-js-element "has-next-subsection")))
      (when (and section (not (= section current-section)))
        (when-let [section-js-element (find-parent-with-class flow-js-element "illustrator-section")]
          (dom/remove-class! section-js-element "illustrator-current")
          (dom/remove-class! section-js-element "narrating-in-subsection"))))
    (when current-flow-js-element
      (dom/add-class! current-flow-js-element "illustrator-current")
      (when-let [subsection-js-element (find-parent-with-class current-flow-js-element "illustrator-subsection")]
        (dom/add-class! subsection-js-element "illustrator-current"))
      (when-let [subsection-frame-js-element (find-parent-with-class current-flow-js-element "illustrator-subsection-frame")]
        (dom/add-class! subsection-frame-js-element "illustrator-current")
        (let [[_ percentage] (get-subsection-index-percentage-tuple @(illustration this) current-flow)
              carousel-js-element (find-parent-with-class current-flow-js-element "illustrator-susbection-carousel")]
          (dom/set-style! carousel-js-element :transform (str "translateX(-" percentage "%)")))
        (when (:subsprev @(current this))
          (dom/add-class! subsection-frame-js-element "has-previous-subsection"))
        (when (:subsnext @(current this))
          (dom/add-class! subsection-frame-js-element "has-next-subsection")))
      (when sections-js-element
        (dom/add-class! section-js-element "illustrator-current")
        (if current-subsection
          (dom/add-class! section-js-element "narrating-in-subsection")
          (dom/remove-class! section-js-element "narrating-in-subsection"))
        (when-let [subsection-frame-js-child (find-child-with-class section-js-element "illustrator-subsection-frame")]
          (when (> (:numsubs @(current this)) 1)
            (dom/add-class! subsection-frame-js-child "has-next-subsection")))))))

(defn fire-event [this id]
  "Dispatch 'timeline' events."
  (let [event (.createEvent js/document "Event")]
    (.initEvent event "timeline" true true)
    (aset event "id" id)
    (.dispatchEvent this event)))

(defn start-replay [this]
  (.log js/console "start-progress-bar")
  (when-let [replay (dom/sel1 (str "#replay-" (instance-id this)))]
    (dom/remove-style! replay :animation)
    (js/setTimeout #(dom/set-style! replay :animation (str "illustration-progress " (:seconds (:flow @(current this))) "s linear")) 100)))

(defn set-keyframe [this new-keyframe]
  (cond
    (< new-keyframe 0)
    (do
      (reset! (keyframe this) 0)
      (reset! (current this) (first @(illustration this)))
      (fire-event this (:id (:flow @(current this)))))
    (>= new-keyframe (count @(illustration this)))
    (do
      (reset! (keyframe this) (- (count @(illustration this)) 1))
      (reset! (current this) (last @(illustration this)))
      (fire-event this (:id (:flow @(current this)))))
    :else
    (do
      (reset! (keyframe this) new-keyframe)
      (reset! (current this) (nth @(illustration this) new-keyframe))
      (fire-event this (:id (:flow @(current this))))))
  (add-remove-classes-and-properties-for-animation this)
  (start-replay this))

(defn clicked-flow [this flow]
  (let [keyframe_of_flow (get-illustration-keyframe @(illustration this) flow)]
    (set-keyframe this keyframe_of_flow)))

(defn selected-flow-by-id [this id]
  (let [keyframe_of_flow (get-illustration-keyframe-by-id @(illustration this) id)]
    (set-keyframe this keyframe_of_flow)))

(defn goto-first-subsection [this]
  "Set keyframe at first subsection flow of current flow"
  (when-let [first-subsection-flow (first (:flows (first (:subsections (:section @(current this))))))]
    (when-let [first-subsection-keyframe (get-illustration-keyframe @(illustration this) first-subsection-flow)]
      (set-keyframe this first-subsection-keyframe))))

(defn goto-previous-subsection [this]
  "Set keyframe at last subsection flow of previous subsection"
  (when-let [subsection-flow (last (:flows (:subsprev @(current this))))]
    (when-let [subsection-keyframe (get-illustration-keyframe @(illustration this) subsection-flow)]
      (set-keyframe this subsection-keyframe))))

(defn goto-next-subsection [this]
  "Set keyframe at first subsection flow of next subsection"
  (when (and (not (:subsnext @(current this)))
             (> (:numsubs @(current this)) 1))
    (goto-first-subsection this))
  (when-let [subsection-flow (first (:flows (:subsnext @(current this))))]
    (when-let [subsection-keyframe (get-illustration-keyframe @(illustration this) subsection-flow)]
      (set-keyframe this subsection-keyframe))))

(defn toggle [this]
  (set-keyframe this @(keyframe this)))

(defn goto-start [this]
  (set-keyframe this 0))

(defn goto-next [this]
  (if (>= (- (count @(illustration this)) 1) (+ @(keyframe this) 1))
    (set-keyframe this (+ @(keyframe this) 1))
    (fire-event this "NEXT_AFTER_LAST")))

(defn goto-previous [this]
  (if (> 0 (- @(keyframe this) 1))
    (fire-event this "PREVIOUS_BEFORE_FIRST")
    (set-keyframe this (- @(keyframe this) 1))))

;;
;; Rendered components
;;

(defn render-buttons [this]
  (let [number-sections (- (count @(illustration this)) 1)]
    [:div.illustrator-buttons {:id (str "illustrator-buttons-" (instance-id this))}
     [:div {:style {:width "104px"
                    :border "2px solid black"
                    :border-radius "12px 0px 0px 12px"
                    :max-height 64
                    :cursor (if (< 0 @(keyframe this)) :pointer :default)}
            :on-click #(when (< 0 @(keyframe this)) (goto-previous this))}
      [:img.illustrator-button
       {:style {:max-height 64
                :opacity (if (< 0 @(keyframe this)) 1 0.4)}
        :src "data:image/svg+xml;base64,
   "
       }]]
     [:div {:style {:border-top "2px solid black"
                    :border-bottom "2px solid black"
                    :max-height 64
                    :cursor (if (= (:seconds (:flow @(current this))) 0) :default :pointer)}
            :on-click #(when (> (:seconds (:flow @(current this))) 0) (toggle this))}
      [:img.illustrator-button
        {:id (str "replay-" (instance-id this))
         :style {:max-height "64px"
                :opacity (if (= (:seconds (:flow @(current this))) 0) 0.4 1)}
         :src "data:image/svg+xml;utf8;base64,"
        }]]
     [:div {:style {:width "15em"
                    :max-height 64
                    :font-size "24px"
                    :background-color (if (> number-sections @(keyframe this)) "#B2FFD7" "white")
                    :opacity (if (> number-sections @(keyframe this)) 1 0.4)
                    :border "2px solid black"
                    :border-radius "0px 12px 12px 0px"
                    :display :flex
                    :align-items :center
                    :justify-content :flex-start
                    :cursor (if (> number-sections @(keyframe this)) :pointer :default)}
            :on-click #(when (> number-sections @(keyframe this)) (goto-next this))}
      [:img.illustrator-button
       {:style {:max-height 64
                :margin-right "0px"}
        :src "data:image/svg+xml;utf8;base64,"
       }]
      [:span {:style {:flex-grow 10
                      :text-align :center
                      :max-height "48px"
                      :overflow :hidden}} 
        [:span {:id "next-button-label"} "NEXT SLIDE"]
        (when @(next-hint this) 
          [:div {:style {:font-size "12px"
                         :color "grey"}} 
            [:span @(next-hint this)]])]]]))


(defn render-sections [this sections]
  [:div.illustrator-sections
    (doall
      (for [section sections]
        [:div.illustrator-section {:key (gensym "n-sct-")
                                   :class [(when (:subsections section) "has-illustrator-subsections")]}
         (doall
           (for [flow (:flows section)]
             [:span.illustrator-flow {:key                     (gensym "n-sct-fl-")
                                      :id                      (get-element-id flow)
                                      :dangerouslySetInnerHTML #js{:__html (:html flow)}
                                      :on-click                #(do (goto-first-subsection this)
                                                                    (clicked-flow this flow))}]))
         [:div.illustrator-subsection-frame
          [:img.illustrator-subsection-frame-left
           {:src      "data:image/svg+xml;base64,
"
            :on-click #(goto-previous-subsection this)}]
          [:img.illustrator-subsection-frame-right
           {:src      "data:image/svg+xml;utf8;base64,"
            :on-click #(goto-next-subsection this)}]
          [:div.illustrator-subsection-frame-hider
           [:div.illustrator-susbection-carousel
            {:style {:width (str (* 100 (count (:subsections section))) "%")}}
            (doall
              (for [subsection (:subsections section)]
                [:div.illustrator-subsection {:key (gensym "n-ssct-")}
                 (doall
                   (for [flow (:flows subsection)]
                     [:span.illustrator-flow {:key                     (gensym "n-sct-fl-")
                                              :id                      (get-element-id flow)
                                              :dangerouslySetInnerHTML #js{:__html (:html flow)}
                                              :on-click                #(clicked-flow this flow)}]))]))]]]]))
    [:div.illustrator-sections-center
     [(keyword (str "div#illustrator-sections-center-overlay" @(id this)))]]])


(defn timeline-render [this sections]
  [:div.illustrator-frame
   [render-sections this sections]
   [render-buttons this]])

(defn goto-section [this id]
  (selected-flow-by-id this id))

(defn render [this illustrator-attrs]
  (when (not (has-attrs this))
    (ctor-attrs this))
  (let [sections @(get illustrator-attrs "sections")
        hint @(get illustrator-attrs "next-hint")
        font-min @(get illustrator-attrs "font-size-min--section")
        font-max @(get illustrator-attrs "font-size-max--section")]
    (reset! (illustration this) (get-illustration sections))
    (reset! (next-hint this) hint)
    [:div {:style {:height "100%" :width "100%"}}
     [:style (css/get-styles font-min font-max @(id this))]
     (timeline-render this sections)]))
